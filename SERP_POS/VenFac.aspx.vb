Public Class VenFac
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Session("Usuario_Aut") = "" Or Session("Destino") <> "VenFac.aspx" Then
        ...
        If Not IsPostBack Then
            ...
            TablaPago.Columns.Add("Recibido")
    ...
    Sub LlenarBuscar()
        ...
        Sql = "SELECT D.CodigoM, D.NombreM, d.RNT, D.FechaDoc, D.FechaVen, D.EncarCod, CASE D.Estado WHEN 'O' THEN '(Abierto)' WHEN 'C' THEN '(Cerrado)' ELSE '(Cancelado)' END Estado, D.SerieCod, D.Comentario, E.Descripcion Pago, D.CodPago, C.CodImpuesto
	            FROM VenFacEnc D
    ...
    Sub TotalPagos()
        ...
        For i As Integer = 0 To Session("tbPago").Rows.Count - 1
            ...
            Cambio += Decimal.Parse(Session("tbPago").Rows(i).Item("Recibido"))
        Next
        Session("tbPago").AcceptChanges()

        lblSaldo.InnerText = FormatNumber(Decimal.Parse(lblTotal.InnerText) - Total)
        lblPagado.InnerText = FormatNumber(Total)
        lblCambio.InnerText = FormatNumber(Cambio - Total)
    End Sub
    ...
    Private Sub BtnAgregarPago_Click(sender As Object, e As EventArgs) Handles btnAgregarPago.Click
        If Decimal.Parse(txtMontoP.Text) = 0 Then
            Exit Sub
        End If

        Dim Fila As DataRow = Session("tbPago").NewRow
        If dlMedio.SelectedItem.Text = "Efectivo" Then
            If Decimal.Parse(txtMontoP.Text) > Decimal.Parse(lblSaldo.InnerText) Then
                Fila("Monto") = FormatNumber(lblSaldo.InnerText)
            Else
                Fila("Monto") = FormatNumber(txtMontoP.Text)
            End If
        ElseIf Decimal.Parse(txtMontoP.Text) <= Decimal.Parse(lblSaldo.InnerText) Then
            Fila("Monto") = FormatNumber(txtMontoP.Text)
        Else
            Exit Sub
        End If
        ...
        Fila("Recibido") = FormatNumber(txtMontoP.Text)
    ...
    Private Sub BtnCrear_Click(sender As Object, e As EventArgs) Handles btnCrear.Click
        ...
        If Decimal.Parse(lblTotal.InnerText) = Decimal.Parse(lblPagado.InnerText) Then
            Estado = "C"
        Else
            Estado = "O"
        End If
        ...
        For I As Integer = 0 To Session("Tabla").Rows.Count - 1
            ...
            Detalle += " INSERT INTO [dbo].[VenFacDet]
           ([NumDoc]
           ,[CodArticulo]
           ,[Descripcion]
           ,[Cantidad]
    ...
    Private Sub BtnAceptarCan_Click(sender As Object, e As EventArgs) Handles btnAceptarCan.Click
        ...
        For I As Integer = 0 To Session("Tabla").Rows.Count - 1
            ...
            Detalle += " INSERT INTO [dbo].[VenNCDet]
           ([NumDoc]
           ,[CodArticulo]
           ,[Descripcion]
           ,[Cantidad]
