Public Class ComFac
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Session("Usuario_Aut") = "" Or Session("Destino") <> "ComFac.aspx" Then
    ...
    Sub BuscarProductos()
        ...
        Sql = "SELECT TOP 100 A.CodArticulo Codigo, A.NomArticulo Descripcion, A.PrecioCompra Precio, B.Stock, A.Costo, A.Fracciones
	            FROM Articulo A
    ...
    Sub LlenarBuscar()
        ...
        Sql = "SELECT D.CodigoM, D.NombreM, d.RNT, D.FechaDoc, D.FechaVen, D.EncarCod, CASE D.Estado WHEN 'O' THEN '(Abierto)' WHEN 'C' THEN '(Cerrado)' ELSE '(Cancelado)' END Estado, D.SerieCod, D.Comentario, E.Descripcion Pago, C.CodPago, C.CodImpuesto
	            FROM ComFacEnc D
                INNER JOIN FormasPago E ON D.CodPago = E.CodPago 
                INNER JOIN Maestro C ON D.CodigoM = C.CodigoM
        ...
        lblLista.InnerText = "Ultimo precio de compra"
        '--------------------------------------------
    ...
    Protected Sub TxtCodigoCliente_TextChanged(sender As Object, e As EventArgs)
        ...
        Sql = "SELECT A.NombreM, A.RTN, 'Ultimo precio de compra' ListNom, C.CodPago, C.Descripcion, CONVERT(Varchar,DATEADD(DD,C.Dias,CONVERT(DATE,GETDATE())),126) Fecha, A.Codimpuesto, A.EncarCod, D.Porcentaje
                FROM Maestro A
    ...
    Protected Sub TxtBuscar_TextChanged(sender As Object, e As EventArgs)
        ...
        Sql = "SELECT TOP 100 A.CodArticulo Codigo, A.NomArticulo Descripcion, PrecioCompra Precio, B.Stock, A.Costo, A.Fracciones
	            FROM Articulo A
	            INNER JOIN ArticuloExis B ON A.CodArticulo = B.CodArticulo
	            WHERE A.Activo = 'Y'
                AND A.Compra = '1'
    ...
    Private Sub GvDetalle_RowDeleting(sender As Object, e As GridViewDeleteEventArgs) Handles gvDetalle.RowDeleting
        If Decimal.Parse(lblPagado.InnerText) <> 0 Then
            lblMsg.Text = "Error: Para eliminar un producto antes elimine el pago"
    ...
    Private Sub BtnCrear_Click(sender As Object, e As EventArgs) Handles btnCrear.Click
        ...
        '-Inicio- Valiaciones
        If Decimal.Parse(lblTotal.InnerText.ToString) = 0 Then
            lblMsg.Text = "Error: Factura no puede ser cero"
            lblMsg.ControlStyle.CssClass = "alert alert-danger"
            Exit Sub
        End If

        If Decimal.Parse(lblPagado.InnerHtml) > Decimal.Parse(lblTotal.InnerHtml) Then
            lblMsg.Text = "Error: El pago no puede ser mayor que el total de la factura"
            lblMsg.ControlStyle.CssClass = "alert alert-danger"
            Exit Sub
        End If

        If lblFormaPago.InnerText = "Contado" And Decimal.Parse(lblPagado.InnerHtml) <> Decimal.Parse(lblTotal.InnerHtml) Then
            lblMsg.Text = "Error: Factura de contado, debe ingresar el pago"
            lblMsg.ControlStyle.CssClass = "alert alert-danger"
            Exit Sub
        End If
        ...
        If Decimal.Parse(lblTotal.InnerText) - Decimal.Parse(lblPagado.InnerText) = 0 Then
            Estado = "C"
        Else
            Estado = "O"
        End If
        ...
        For I As Integer = 0 To Session("Tabla").Rows.Count - 1
            ...
            Detalle += " INSERT INTO [dbo].[ComFacDet]
           ([NumDoc]
           ,[CodArticulo]
           ,[Descripcion]
           ,[Cantidad]
     ...
     VALUES
           (''NumSig''
           ,''" + Session("Tabla").Rows(I).Item("Codigo").ToString + "''
           ,''" + Session("Tabla").Rows(I).Item("Producto").ToString + "''
           ...
           ,''" + Session("Usuario_Aut") + "'') "
        Next
        '-Fin- Lineas o Detalle de docuemnto

        '-Inicio- Detalle de pago
        For I As Integer = 0 To Session("tbPago").Rows.Count - 1
            Pago += " INSERT INTO [dbo].[BanEfeDet2]
           ([NumDoc]
           ,[Monto]
           ,[Medio]
           ,[Referencia]
     ...
     VALUES
        ...
        ,@ValorApli = '" + Replace(lblPagado.InnerHtml.ToString, ",", "") + "'
        ,@TransID = '0'
    ...
    Private Sub BtnAceptarCan_Click(sender As Object, e As EventArgs) Handles btnAceptarCan.Click
        ...
        If Decimal.Parse(lblTotal.InnerText.ToString) = 0 Then
            lblMsg.Text = "Error: NC no puede ser cero"
            lblMsg.ControlStyle.CssClass = "alert alert-danger"
            Exit Sub
        End If

        Sql = "SELECT TOP 1 A.ID IdCorre, A.Descripcion, A.NumSig, Info
                FROM CorrelaDet A 
				INNER JOIN CorrelaEnc B ON A.IdCorre = B.IdCorre
                WHERE A.Activo = 'Y'
                AND B.ObjectCode = '2'"
        ...
        Numeracion = Datos.Tables(0).Rows(0).Item("IdCorre")
        ...
        For I As Integer = 0 To Session("Tabla").Rows.Count - 1
            ...
            Detalle += " INSERT INTO [dbo].[ComNCDet]
           ([NumDoc]
           ,[CodArticulo]
           ,[Descripcion]
           ,[Cantidad]
     ...
     VALUES
        ...
        Sql = "EXEC SP_ComNC
           ...
           ,'" + txtCodigo.Text + "'
           ,'" + txtNombre.Value + "'
           ,'" + Session("CodPago").ToString + "' 
           ,'" + dlVendedor.SelectedValue + "'
           ,'" + Session("Usuario_Aut") + "'
           ,'" + Estado + "'
           ,'" + Session("CopiaNumDoc") + "'
           ,'7'
