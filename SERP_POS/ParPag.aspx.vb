...
Public Class ParPag
    ...
    Sub LlenarLista()
        ...
        Sql = "SELECT 0 Codigo, '...' Descripcion, '0' Dias UNION ALL SELECT CodPago, Descripcion, Dias
                FROM FormasPago"
    ...
    Private Sub GvEncargado_RowUpdating(sender As Object, e As GridViewUpdateEventArgs) Handles GvEncargado.RowUpdating
        ...
        If CType(Fila.FindControl("Codigo"), Label).Text.TrimEnd = "0" Then
            Sql = "INSERT INTO FormasPago (Descripcion, Dias) VALUES 
                   ('" + CType(Fila.FindControl("Descripcion"), TextBox).Text.TrimEnd + "', 
                    '" + CType(Fila.FindControl("Dias"), TextBox).Text.TrimEnd + "') "
            Conf.EjecutaSql(Sql)

            LlenarLista()
        Else
            Sql = "UPDATE FormasPago SET  Descripcion = '" + CType(Fila.FindControl("Descripcion"), TextBox).Text.TrimEnd + "',
                                          Dias = '" + CType(Fila.FindControl("Dias"), TextBox).Text.TrimEnd + "'
            ...
            Session("GvEncargado").Rows(e.RowIndex).Item("Dias") = CType(Fila.FindControl("Dias"), TextBox).Text.TrimEnd
        End If
