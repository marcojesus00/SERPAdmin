...
Public Class ParAlm
    ...
    Sub LlenarLista()
        ...
        Sql = "SELECT '0' ID, '...' AlmacenCod, '' Descripcion, '' Pais, '' Ciudad, '' Departamento, 'Y' Activo
                UNION ALL
               SELECT ID, AlmacenCod, Descripcion, Pais, Ciudad, Departamento, Activo
                  FROM Almacen ORDER BY ID"
    ...
    Private Sub GvEncargado_RowUpdating(sender As Object, e As GridViewUpdateEventArgs) Handles GvEncargado.RowUpdating
        ...
        Sql = Sql + "EXEC SP_Almacen '" + Tipo + "','" +
                 CType(Fila.FindControl("AlmacenCod"), TextBox).Text.TrimEnd + "', 
            '" + CType(Fila.FindControl("Descripcion"), TextBox).Text.TrimEnd + "',                                        
            '" + CType(Fila.FindControl("Pais"), TextBox).Text.TrimEnd + "',
            '" + CType(Fila.FindControl("Ciudad"), TextBox).Text.TrimEnd + "',
            '" + CType(Fila.FindControl("Departamento"), TextBox).Text.TrimEnd + "',
