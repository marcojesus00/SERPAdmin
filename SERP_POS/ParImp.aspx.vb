...
Public Class ParImp
    ...
    Sub LlenarLista()
        ...
        Sql = "SELECT 0 Codigo, '...' Descripcion, '0' Porcentaje UNION ALL SELECT CodImpuesto, Descripcion, Porcentaje
                FROM Impuestos"
    ...
    Private Sub GvEncargado_RowUpdating(sender As Object, e As GridViewUpdateEventArgs) Handles GvEncargado.RowUpdating
        ...
        If CType(Fila.FindControl("Codigo"), Label).Text.TrimEnd = "0" Then
            Sql = "INSERT INTO Impuestos (Descripcion, Porcentaje) VALUES 
                   ('" + CType(Fila.FindControl("Descripcion"), TextBox).Text.TrimEnd + "', 
                    '" + CType(Fila.FindControl("Porcentaje"), TextBox).Text.TrimEnd + "') "
            Conf.EjecutaSql(Sql)

            LlenarLista()
        Else
            Sql = "UPDATE Impuestos SET  Descripcion = '" + CType(Fila.FindControl("Descripcion"), TextBox).Text.TrimEnd + "',
                                         Porcentaje = '" + CType(Fila.FindControl("Porcentaje"), TextBox).Text.TrimEnd + "'
                   WHERE CodPago = '" + CType(Fila.FindControl("Codigo"), Label).Text.TrimEnd + "'"
            Datos = Conf.EjecutaSql(Sql)

            Session("GvEncargado").Rows(e.RowIndex).Item("Descripcion") = CType(Fila.FindControl("Descripcion"), TextBox).Text.TrimEnd
            Session("GvEncargado").Rows(e.RowIndex).Item("Porcentaje") = CType(Fila.FindControl("Porcentaje"), TextBox).Text.TrimEnd
        End If
