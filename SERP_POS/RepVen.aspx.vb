...
Public Class RepVen
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Session("Usuario_Aut") = "" Or Session("Destino") <> "RepVen.aspx" Then
    ...
    Protected Sub txtBuscar_TextChanged(sender As Object, e As EventArgs)
        ...
        If dlFecha.SelectedValue = "Fecha Sis" Then
            Sql = "SELECT 
                ...
                CONVERT(VARCHAR(50), CAST(B.Costo AS MONEY ),1) Costo, 
                CONVERT(VARCHAR(50), CAST(B.Cantidad*B.PrecioConDes AS MONEY ),1) [Total Linea], 
                CONVERT(VARCHAR(50), CAST(B.Cantidad*B.Costo AS MONEY ),1) [Total Costo], 
                CONVERT(VARCHAR(50), CAST(B.Cantidad*(B.PrecioConDes-B.Costo) AS MONEY ),1) [Total Ganancia], 
                C.EncarNom Vendedor
	                FROM VenFacEnc A
	                INNER JOIN VenFacDet B ON A.NumDoc = B.NumDoc
            ...
            UNION ALL
               ...
               -B.Cantidad, 
                ...
                C.EncarNom Vendedor
	                FROM VenNcEnc A
	                INNER JOIN VenNcDet B ON A.NumDoc = B.NumDoc
	                INNER JOIN Encargado C ON A.EncarCod = C.EncarCod
	                WHERE A.EncarCod LIKE '%" + dlAlmacen.SelectedValue + "%'
                    AND A.CodigoM + A.NombreM LIKE '%" + txtBuscar.Text + "%'
                    AND CONVERT(DATE,A.FechaSis) BETWEEN '" + txtF1Reim.Text + "' AND '" + txtF2Reim.Text + "' 
        ...
        Else
            ...
            UNION ALL
                ...
                C.EncarNom Vendedor
                    ...
                    AND A.CodigoM + A.NombreM LIKE '%" + txtBuscar.Text + "%'                   
                    AND CONVERT(DATE,A.FechaDoc) BETWEEN '" + txtF1Reim.Text + "' AND '" + txtF2Reim.Text + "' 
                    ORDER BY A.FechaDoc ASC, A.NumDoc ASC"
        End If
    ...
    Private Sub GvPrincipal_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles GvPrincipal.RowDataBound
        If ((e.Row.RowType = DataControlRowType.DataRow) And (e.Row.RowType <> DataControlRowType.EmptyDataRow)) Then
            ...
            TotalCosto += Convert.ToDecimal(Fila.Item("Total Costo"))
            TotalGanacia += Convert.ToDecimal(Fila.Item("Total Ganancia"))
            TotalDocumento += 1

            lblTotalDocumento.Text = "Facturas: " + Format(TotalDocumento, "#,##0")
            lblTotalVenta.Text = "Venta: " + Format(TotalVenta, "#,##0.00")
            lblTotalCosto.Text = "Costo: " + Format(TotalCosto, "#,##0.00")
            lblTotalGanancia.Text = "Ganacia: " + Format(TotalGanacia, "#,##0.00")

            If Decimal.Parse(e.Row.Cells(13).Text.ToString) <= 0.00 Then
                e.Row.Cells(13).ControlStyle.ForeColor = System.Drawing.Color.Red
            Else
                e.Row.Cells(13).ControlStyle.ForeColor = System.Drawing.Color.Blue
            End If
        End If
    End Sub
    ...
    Private Sub btnExcel_Click(sender As Object, e As EventArgs) Handles btnExcel.ServerClick
        GVtoExcel(Session("GV").Tables(0), "Ventas " + DateTime.Now.ToString("yyyy-MM-dd"))
    End Sub
