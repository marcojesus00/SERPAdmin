...
Public Class ParGArt
    ...
    Sub LlenarLista()
        ...
        Sql = "SELECT 0 Codigo, '...' Nombre, '' Descripcion, 'Y' Activo UNION ALL SELECT IdGrupCod Codigo, Prefijo Nombre, Descripcion, Activo
                FROM GrupoArtEnc"
    ...
    Private Sub GvEncargado_RowUpdating(sender As Object, e As GridViewUpdateEventArgs) Handles GvEncargado.RowUpdating
        ...
        If CType(Fila.FindControl("Codigo"), Label).Text.TrimEnd = "0" Then
            Sql = "INSERT INTO GrupoArtEnc (Prefijo, Descripcion, Activo) VALUES 
                   ('" + CType(Fila.FindControl("Prefijo"), TextBox).Text.TrimEnd + "', 
                    '" + CType(Fila.FindControl("Descripcion"), TextBox).Text.TrimEnd + "',                 
                    '" + CType(Fila.FindControl("DlActivo"), DropDownList).SelectedValue.TrimEnd + "') "
            Conf.EjecutaSql(Sql)

            LlenarLista()
        Else
            Sql = "UPDATE GrupoArtEnc SET Prefijo = '" + CType(Fila.FindControl("Prefijo"), TextBox).Text.TrimEnd + "',
                                          Descripcion = '" + CType(Fila.FindControl("Descripcion"), TextBox).Text.TrimEnd + "',
                                          Activo = '" + CType(Fila.FindControl("DlActivo"), DropDownList).SelectedValue.TrimEnd + "'
                WHERE IdGrupCod = '" + CType(Fila.FindControl("Codigo"), Label).Text.TrimEnd + "'"
            Datos = Conf.EjecutaSql(Sql)

            Session("GvEncargado").Rows(e.RowIndex).Item("Nombre") = CType(Fila.FindControl("Prefijo"), TextBox).Text.TrimEnd
            Session("GvEncargado").Rows(e.RowIndex).Item("Descripcion") = CType(Fila.FindControl("Descripcion"), TextBox).Text.TrimEnd
    ...
    Private Sub GvEncargado_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles GvEncargado.RowDataBound
        If ((e.Row.RowType = DataControlRowType.DataRow) And (e.Row.RowType <> DataControlRowType.EmptyDataRow)) Then
            If e.Row.RowIndex = 0 Then
                ...
                e.Row.Cells(3).ControlStyle.ForeColor = System.Drawing.Color.Transparent
            End If
        End If
    End Sub
End Class